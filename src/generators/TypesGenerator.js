...
export class TypesGenerator {
	...
	async generate() {
		...
		console.log(chalk.green(`✅ 3단계 완료: TypeScript 타입 파일 생성됨 (${assetInfo.length}개 Asset)`))
		return [outputPath]
	}
	...
	createAssetInfo(filePath, assetDir) {
		...
		const pathParts = directory === '.' ? [] : directory.split('/').filter(part => part)
	...
	generateTypeDefinitions(assetInfo) {
		const { assetNameType, assetPropsType, pathMapName } = this.config.typeGeneration
...
export const colorMapping: Record<ColorType, string> = ${JSON.stringify(this.config.colorMapping, null, 2)}`
	...
	generateAssetPathMap(assetInfo, mapName) {
		...
		const entries = assetInfo.map(asset => {
			return `  '${asset.name}': {
    name: '${asset.name}',
...
export type AssetNameByCategory<T extends string> = ${assetNameType} // 실제로는 카테고리별 필터링해야 함`
	...
	async handleFileOverwrite(filepath, filename) {
		try {
			...
			const overwriteMode = this.config.fileGeneration?.overwriteMode || 'overwrite'
