...
export class AssetGenerator {
  ...
  async generate() {
    try {
      ...
      console.log(
        chalk.blue(
          '📋 올바른 실행 순서: 폴더명 제거 → 새 네이밍 → 타입 생성 → 컴포넌트 생성'
        )
      );
      ...
      console.log(
        chalk.blue(`📊 실행된 단계: ${completedSteps}/${enabledSteps.length}`)
      );
  ...
  async executeStep(step, config) {
    ...
    switch (step.flagKey) {
      ...
      case 'generateUtils':
        ...
        const utilsResult = await utilsGen.generate(outputDir);
        return { generatedFiles: [utilsResult] };
  ...
  getEnabledSteps(config) {
    ...
    const ALL_STEPS = [
      ...
      {
        name: 'Asset Utils 생성',
        description: 'Asset 유틸리티 함수들 생성',
        flagKey: 'generateUtils',
        required: false,
      },
      {
        name: 'Asset 컴포넌트 생성',
        description: 'React Asset 컴포넌트, hooks 생성',
